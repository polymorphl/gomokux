...
void Graphic::drawZoneEat() {
    ...
    SDL_FillRect(_screen, &(zoneOne), SDL_MapRGB(_screen->format, 102, 72, 72));
    zoneTwo.x = _width - 15 - BLOCK_SIZE;
    zoneTwo.y = 155;
    zoneTwo.h = 10 * (BLOCK_SIZE + 10);
    zoneTwo.w = BLOCK_SIZE;
    SDL_FillRect(_screen, &(zoneTwo), SDL_MapRGB(_screen->format, 102, 72, 72));
}
...
void Graphic::drawSquare() {
    ...
    desk.x = 60;
    desk.y = 60;
    desk.h = (18 * BLOCK_SIZE) + 32;
    desk.w = (18 * BLOCK_SIZE) + 32;
    SDL_FillRect(_screen, &(desk), SDL_MapRGB(_screen->format, 102, 72, 72));        
    drawLine();
    drawPawn();
    drawEat();
    drawRule();
...
void Graphic::launchGame() {
    ...
    std::cout << RED << "[START] Game begin !" << END << std::endl;
    if (_playerTwo->getPlayerType() == IA)
        ai = new AI(static_cast<t_ia_type>(_playerTwo->getType()));
    ...
    while (this->_winner == 0) {
        ...
        if (checkPresentPlayer(activPlayer) == HUMAN) {
            ...
            std::cout << BLUE << "Ia play" << END << std::endl;
            while (preparCheckRule(activPlayer) == 1) {
                retIa = ai->launchAI(_map, activPlayer, _playerOne->getNbPion(), _playerOne->getNbPion());
    ...
    std::cout << RED << "[END] Winner team " << _winner - 1 << " !" << END << std::endl;
}
